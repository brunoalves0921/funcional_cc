...
consulta indice armarios
    | Map.member indice armarios = Just $ fst $ fromJust $ Map.lookup indice armarios
    | otherwise = Nothing

-- dado o número do armário, retorna Right armário ou Left "Armario nao existe"
pegarArmario :: Int -> Armarios -> Either String Armario
pegarArmario indice armarios = case consulta indice armarios of
    Just estado -> Right $ fromJust $ Map.lookup indice armarios
    Nothing -> Left $ "Armario " ++ codigo ++ " nao existe"
    where codigo = show indice
...
pegarCodigo indice armarios = case pegarArmario indice armarios of
    Right(Livre, codigo) -> Right codigo
    Right(Alugado, codigo) -> Left $ "Armario " ++ show indice ++ " ja esta ocupado"
    Left msg -> Left msg
...
aluguel indice armarios = case pegarCodigo indice armarios of
    Right codigo -> case pegarArmario indice armarios of
                        Right (Livre, codigo) -> Right $ Map.insert indice (Alugado, codigo) armarios
                        _ -> Left $ "Armario " ++ show indice ++ " ja esta ocupado"
    Left msg -> Left msg
...
devolucao indice codigo armarios = case pegarArmario indice armarios of
    Right (Alugado, c) -> if codigo == c then Right $ Map.insert indice (Livre, codigo) armarios else Left "Codigo incorreto"
    Right (Livre, c) -> Left "Armario ja esta livre"
    Left msg -> Left msg
    where estado = fst (fromRight (Alugado, "") $ pegarArmario indice armarios)
{-
if 0 ==7 then Right $ Map.insert indice (Livre, codigo) armarios
                        else Left $ "Armario " ++ show indice ++ " ja esta livre"-}
lockers :: Armarios
